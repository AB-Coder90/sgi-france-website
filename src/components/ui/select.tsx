...
const Select = React.forwardRef<HTMLButtonElement, SelectProps>(
  ({ className, items, error, children, onValueChange, ...props }, ref) => {
    return (
      <div className="space-y-2">
        <SelectPrimitive.Root {...props} onValueChange={onValueChange}>
          <SelectTrigger ref={ref} className={cn(error && "border-red-500", className)}>
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-md border bg-dark-800/50",
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 min-w-[8rem] overflow-hidden rounded-md border bg-dark-800/90",
        "text-popover-foreground shadow-md data-[state=open]:animate-in",
        "data-[state=closed]:animate-out data-[state=closed]:fade-out-0",
        "data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95",
        "data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2",
        "data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2",
        "data-[side=top]:slide-in-from-bottom-2 backdrop-blur-sm",
        position === "popper" &&
